...
const definitionIndex = new Map(
  definitions.map((entry) => [entry.term, entry]),
);
...
> = {
  Established: {
    ...
    card: "border-emerald-200/70 bg-emerald-50/70 hover:bg-emerald-50",
  },
  Emerging: {
    chip: "border-amber-300 bg-amber-100 text-amber-950",
    dot: "bg-amber-500",
    text: "Growing adoption, still evolving",
    card: "border-amber-200/70 bg-amber-50/70 hover:bg-amber-50",
  },
  Experimental: {
    chip: "border-violet-300 bg-violet-100 text-violet-950",
    dot: "bg-violet-500",
    text: "Limited deployment, pilots, or research",
    card: "border-violet-200/70 bg-violet-50/70 hover:bg-violet-50",
  },
};
...
export function AiMlLibraryBrowser() {
  ...
  const displayedApplication = selectedMatchesFilter
    ? selectedApplication
    : null;
  const selectedDefinition = activeTerm
    ? definitionIndex.get(activeTerm)
    : null;
    ...
    <div className="group relative h-full">
      ...
      >
        <CardHeader className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              className="relative z-20 inline-flex h-6 items-center rounded-4xl border border-transparent bg-white px-2.5 py-0.5 text-xs font-medium text-foreground transition-colors hover:border-foreground focus-visible:border-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          The drawer opens here when you click a card. Use the term chips to
          pull up short definitions as you browse.
        </p>
      </div>
      <div className="rounded-2xl border border-dashed border-border/70 bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
        Start with any path, then layer the filters on top when you want to
        narrow the list.
      </div>
    </div>
  );
        ...
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="space-y-1">
            <Badge variant="outline">
              {definitionTypeLabel(definition.type)}
            </Badge>
...
function MaturityLegend() {
  return (
    <p className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
      <span className="font-medium">Maturity key:</span>
      {maturityOrder.map((value) => (
        <span className="inline-flex items-center gap-1.5" key={value}>
          <span
            aria-hidden="true"
            className={cn("size-2 rounded-full", maturityMeta[value].dot)}
          />
          {value}
        </span>
      ))}
...
function definitionTypeLabel(type: string) {
  switch (type) {
    case "technology":
      return "Technology";
    case "key-concern":
